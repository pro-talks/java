...
После `minor collections` размер включает некоторые объекты, которые являются мусором (больше не живы), но не могут быть возвращены. Эти объекты либо содержатся в заемном поколении, либо ссылаются на заемное поколение.
...
Пропускная способность и занимаемая площадь лучше всего измеряются с помощью показателей, специфичных для приложения. Например, пропускная способность веб-сервера может быть проверена с использованием генератора нагрузки клиента, тогда как занимаемая площадь сервера может быть измерена в операционной системе Solaris с помощью команды pmap. Однако паузы из-за сборки мусора легко оцениваются путем проверки диагностического вывода самой виртуальной машины.
...
**Определение размера одного поколения не влияет на частоту сбора и время паузы для другого поколения.**
