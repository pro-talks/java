...
Параллельный сборщик (`The Parallel Collector` or `The Throughput Collector`) работает также как и последовательный коллектор, основное отличие в том, что для ускорения сбора мусора используется несколько потоков.
...
- **Ядер = 1**. На машине с одним ядром параллельный сборщик, не будет работать также хорошо как и последовательный сборщик, из-за накладных расходов, необходимых для параллельного выполенения (например, синхронизации).
...
- А также при использовани параллельного-сборщика может наблюдаться эффект фрагментации. Когда несколько потоков обрабатывают `young generation` и передвигают объекты в `tenured generation`, эти потоки резервируют части памяти (`"promotion buffers"`) в `ternured generation`, данный процесс может стать прициной фрагментации. 
...
Статистика, такая как среднее время паузы, сохраняемое сборщиком, обновляется в конце каждой коллекции. Затем выполняются тесты для определения того, были ли достигнуты цели, и вносятся необходимые корректировки в размер поколения. Исключением является то, что явные сборки мусора (например, вызовы System.gc ()) игнорируются с точки зрения ведения статистики и корректировки размеров поколений
...
Если размер `Heap` настроен через командную строку и подходит для 





Однако включение параллельного коллектора должно сделать паузы сбора короче. 

Поскольку несколько потоков сборщика мусора участвуют в незначительном сборе, возможна некоторая фрагментация из-за продвижения от молодого поколения к постоянному поколению во время сбора. Каждый поток сбора мусора, участвующий в вспомогательной сборке, резервирует часть штатного поколения для продвижения по службе, и разделение доступного пространства на эти «буферы продвижения» может вызвать эффект фрагментации. Уменьшение количества потоков сборщика мусора и увеличение размера заемного поколения уменьшит этот эффект фрагментации.
...
Включитьв `-XX: + UseParallelGC`



Параллельный коллектор включается с параметром командной строки . По умолчанию при использовании этой опции как второстепенные, так и основные коллекции выполняются параллельно, чтобы еще больше снизить затраты на сборку мусора.
