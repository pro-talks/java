...
Оставить все объекты, которые доступны из работающей программы. Все остальные объекты - мусор.
...
- Ось Y - это общее количество байтов для объектов с соответствующим временем жизни. (**CHECK**)
...
#### Сборку мусора в поколениях

- Эффективный сбор возможен благодаря тому, что большинство объектов «умирают молодыми».
- Чтобы оптимизировать этот сценарий, память управляется поколениями (пулы памяти, содержащие объекты разного возраста).

- Сборка мусора происходит в каждом поколении, когда поколение заполняется.
- Подавляющее большинство объектов размещено в пуле, посвященном молодым объектам (молодому поколению), и большинство объектов погибает там.
- Когда молодое поколение заполняется, это вызывает незначительную коллекцию, в которой собирается только молодое поколение; мусор в других поколениях не утилизируется.
- Незначительные коллекции могут быть оптимизированы, если предположить, что гипотеза слабого поколения верна, и большинство объектов в молодом поколении являются мусором и могут быть восстановлены.
- Стоимость таких коллекций, в первом порядке, пропорциональна количеству собираемых живых объектов; молодое поколение, полное мертвых предметов, собирается очень быстро.
- Как правило, некоторая часть уцелевших объектов из молодого поколения передается постоянному поколению во время каждой незначительной коллекции.
- В конце концов, штатное поколение заполнится и должно быть собрано, в результате чего будет получена основная коллекция, в которой будет собрана вся куча.
- Основные коллекции обычно длятся намного дольше, чем второстепенные, потому что задействовано значительно большее количество объектов.





- Как отмечено в разделе «Эргономика», эргономика динамически выбирает сборщик мусора для обеспечения высокой производительности в различных приложениях.

- Серийный сборщик мусора предназначен для приложений с небольшими наборами данных, и его параметры по умолчанию были выбраны, чтобы быть эффективными для большинства небольших приложений.

- Параллельный или пропускной сборщик мусора предназначен для использования с приложениями, которые имеют средние и большие наборы данных.

- Параметры размера кучи, выбранные с помощью эргономики, а также функции политики адаптивного размера, предназначены для обеспечения высокой производительности серверных приложений.

- Эти варианты хорошо работают в большинстве, но не во всех случаях, что приводит к центральному принципу этого документа:

  - Если сборка мусора становится узким местом, вам, скорее всего, придется настраивать общий размер кучи, а также размеры отдельных поколений.

  - Проверьте подробные выходные данные сборщика мусора, а затем изучите чувствительность вашей индивидуальной метрики производительности к параметрам сборщика мусора.

    

**Расположение поколений по умолчанию, за исключением параллельного коллектора и G1**

![](https://docs.oracle.com/javase/8/docs/technotes/guides/vm/gctuning/img/jsgct_dt_001_armgnt_gn.png)

- При инициализации максимальное адресное пространство фактически резервируется, но не выделяется физической памяти, если только это не требуется.
- Полное адресное пространство, зарезервированное для памяти объекта, можно разделить на молодое и постоянное поколение.
- Молодое поколение состоит из Эдема и двух пространств выживших. Большинство объектов изначально расположены в Eden. Одно пространство для выживших в любое время пусто и служит местом назначения любых живых объектов в Эдеме; другое место оставшегося в живых является местом назначения во время следующего копирования коллекции. Таким образом, объекты копируются между оставшимися в живых пространствами до тех пор, пока они не станут достаточно взрослыми, чтобы их можно было использовать (копируются в постоянное поколение).
