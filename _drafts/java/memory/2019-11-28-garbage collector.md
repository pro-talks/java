...
# Garbage-First Garbage Collector TODO

"The Garbage-First (G1)" - это сборщик мусора серверного типа, предназначенный для многопроцессорных машин с большим объемом памяти. 

Он пытается достичь целей минимальной паузы с высокой вероятностью при достижении высокой пропускной способности.

`Whole-heap operations`, такие как глобальная маркировка, выполняются одновременно с потоками приложения. 

Это предотвращает пробелему длителных прерываний, пропорциональных размеру `heap` или живый данных.
...
G1 делит кучу на области фиксированного размера (серые прямоугольники), как показано на рисунке 9-1, «Деление кучи по G1».
...
G1 является относится к поколениям `generational` в логическом смысле.

Множество пустых областей обозначено как логическое молодое поколение. 

На рисунке молодое поколение светло-голубого цвета.

`Allocations` делаются из этого логического `young generation`, и когда `young generation` полно, этот набор регионов отчищается в молодом поколении дешевом (`young collection`).
