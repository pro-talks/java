...
Коллектор CMS может использоваться в режиме, в котором `concurrent phases` выполняются `incrementally`.
...
- Одновременно отслеживает граф достижимых объектов, используя один или несколько процессоров, пока выполняются потоки приложения.

- Одновременно восстанавливает разделы графа объектов, которые были изменены с момента трассировки на предыдущем шаге, используется один процессор.
...
Как правило, сборщик CMS использует один или несколько процессоров в течение всей фазы одновременного отслеживания, без добровольного отказа от них.

Точно так же один процессор используется для всей параллельной фазы развертки, опять же, не отказываясь от него.
...
Обратите внимание, что выходные данные для сборщика CMS чередуются с выходными данными из `minor collections` ; как правило, многие `minor collections` убираются во `concurrent collection`.
